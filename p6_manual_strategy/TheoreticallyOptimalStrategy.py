...
class TheoreticallyOptimalStrategy(object):
    ...
    def test_policy(self, symbol='AAPL', sd=dt.datetime(2010, 1, 1), ed=dt.datetime(2011, 12, 31), sv=100000):
        ...
        tos_orders.fillna(value=pd.Series({'Symbol': symbol, 'Order': 'BUY', 'Shares': 0}), axis=0,
                          inplace=True)  # replace NaNs with empty BUY orders
    ...
    def evaluate(self):
        ...
        benchmark_portval = compute_portvals(benchmark_orders, 100000, 0.0,
                                             0.0)  # ignore commission and market impact for theoretically optimal strategy
        ...
        portval_graph = portval_df.plot(title="Benchmark & Theoretically Optimal Portfolio Value - alata6", fontsize=12,
        ...
        tos_cr = (tos_portval.iloc[-1].at['Theoretically Optimal Strategy PortVal'] / tos_portval.iloc[0].at[
            'Theoretically Optimal Strategy PortVal']) - 1
