...
def produce_chart_number(win_prob, figure_number, figure_title, number_of_simulations=1, bankroll=INFINITE_BANKROLL, legend_position='lower right'):
    ...
    if figure_number == 1:
        ...
        episodes_result_array = run_multiple_simulation_episodes(win_prob, number_of_simulations)
        ...
        std_winnings_for_spins = np.std(episodes_result_array, axis=0, ddof=1)
    ...
    np.savetxt('Experiment_{}'.format(figure_number), np.unique(episodes_result_array[:, -1], return_counts=True), fmt='%i', delimiter=",")
    # Charts must be generated by the code (including any desired annotations) and saved as .png files to the project directory in the images subdirectory
...
def test_code():
    ...
    produce_chart_number(win_prob, 1, "Exp1 : 10 episodes w/o bankroll - alata6", number_of_simulations=10)
    produce_chart_number(win_prob, 2, "Exp1 : Mean and Std Dev for 1000 episodes w/o bankroll - alata6", number_of_simulations=1000)
    produce_chart_number(win_prob, 3, "Exp1 : Median and Std Dev for 1000 episodes w/o bankroll - alata6", number_of_simulations=1000)
    produce_chart_number(win_prob, 4, "Exp2 : Mean and Std Dev for 1000 episodes with bankroll - alata6", bankroll=256, number_of_simulations=1000)
    produce_chart_number(win_prob, 5, "Exp2 : Median and Std Dev for 1000 episodes with bankroll - alata6", bankroll=256, number_of_simulations=1000)
